...
public class GetDvdMovieInformationById(int id) : IRequest<Dvd.DvdMovieInformation?>
{
    public int Id { get; } = id;

    public class GetDvdMovieInformationByIdQueryHandler(IHttpClientFactory httpClientFactory, IAppJsonDeserializer appJsonDeserializer) : IRequestHandler<GetDvdMovieInformationById, Dvd.DvdMovieInformation?>
    {
        ...
        public async Task<Dvd.DvdMovieInformation?> Handle(GetDvdMovieInformationById request, CancellationToken cancellationToken)
        {
            if (_client == null)
            ...
            try
            {
                HttpResponseMessage response = await _client.GetAsync($"api/dvdmovieinformation/{request.Id}", cancellationToken);
