...
public class GetAllDvdMovieInformationByNames(List<string> names) : IRequest<List<Dvd.DvdMovieInformation>?>
{
    public List<string> Names { get; } = names;

    public class GetAllDvdMovieInformationByNamesQueryHandler(IHttpClientFactory httpClientFactory, IAppJsonDeserializer appJsonDeserializer) : IRequestHandler<GetAllDvdMovieInformationByNames, List<Dvd.DvdMovieInformation>?>
    {
        ...
        public async Task<List<Dvd.DvdMovieInformation>?> Handle(GetAllDvdMovieInformationByNames request, CancellationToken cancellationToken)
        {
            if (_client == null)
            ...
            try
            {
                HttpResponseMessage response = await _client.GetAsync($"api/dvdmovieinformation/movie/{request.Names}", cancellationToken);
