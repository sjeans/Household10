...
public class GetEpisodeListByShowIdAsync(int showId) : IRequest<List<Episode>?>
{
    public int ShowId { get; } = showId;

    public class GetAllEpisodesByShowIdAsyncQueryHandler(IHttpClientFactory httpClientFactory, IMapper mapper, IAppJsonDeserializer appJsonDeserializer) : IRequestHandler<GetEpisodeListByShowIdAsync, List<Episode>?>
    {
        ...
        public async Task<List<Episode>?> Handle(GetEpisodeListByShowIdAsync request, CancellationToken cancellationToken)
        {
            if (_client == null)
            ...
            try
            {
                HttpResponseMessage response = await _client.GetAsync($"api/episodedetail/showinformation/{request.ShowId}", cancellationToken);
